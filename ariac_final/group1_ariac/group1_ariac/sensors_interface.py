...
from group1_ariac.retrieve_orders_interface import Color
...
from group1_ariac_msgs.msg import ConveyorPartsPoseLot, ConveyorPartsPose

class SensorsInterface(Node):
    def __init__(self, node_name='sensors_interface'):
        super().__init__(node_name)

        # information on parts detected on the conveyor
        self.new_conveyor_part = False
        self.new_part = True

        # !!! get rid of this eventually, or the same veriable in locater_interface
        ...
        # Conveyor RGB Camera Pose w.r.t world frame --> [(x, y, z), (roll, pitch, yaw)]
        self._conveyor_camera_pose = [[-0.681511, 3.893961, 1.918427],(0.0, np.pi/2, np.pi)]
        ...
        # Part Pose Reporting Object for bins
        self.part_bin_poses = {
        ...
        # Part Pose Reporting Object for conveyor
        self.part_conveyor_poses = self.part_bin_poses.copy()
        
        # Center of Part Poses for bin
        ...
        self.color_mapping = {
            ...
            "green"  : PartMsg.GREEN,
            "blue"   : PartMsg.BLUE,
            "orange" : PartMsg.ORANGE,
            "purple" : PartMsg.PURPLE
        }

        self.type_mapping = {
            "battery"   : PartMsg.BATTERY,
            "pump"      : PartMsg.PUMP,
            "sensor"    : PartMsg.SENSOR,
            "regulator" : PartMsg.REGULATOR
        }
        ...
        self.conveyor_RGB_camera_sub = self.create_subscription(ImageMsg,
                                                                  "/ariac/sensors/conveyor_RGB_camera/rgb_image",#'/ariac/sensors/{sensor_name}/rgb_image'
                                                                  self._conveyor_RGB_camera_cb,
                                                                  qos_profile_sensor_data,
                                                                  callback_group=self.sensors_cb_group
                                                                  )
        
        # Conveyor RBG Camera Sub
        ...
        # Create 'PartsPoses' Object to store and retrieve pose information of the slots in each bins
        self.partsbinsposes = PartsPoses()
        self.partsconveyorposes = PartsPoses()
        ...
        self._part_conveyor_pose_publisher = self.create_publisher(ConveyorPartsPoseLot, "group1_ariac/conveyor_part_poses",10)
        self._part_conveyor_pose_timer = self.create_timer(1.0, self.conveyor_predictor_timer_cb)

    # update the predicted locations of known parts on the conveyor
    def conveyor_predictor_timer_cb(self):
        self.log_parts_in_bins()

        msg = ConveyorPartsPoseLot()
        conveyor_parts_msg = ConveyorPartsPose()

        #self.get_logger().info("Updating predictor")
        for i in range(0, len(self.conveyor_parts)):
            part = self.conveyor_parts[i].copy()

            # Part pose message to store pose of part
            part_pose_msg = PartPose()
            
            part_pose_msg.part.color = self.color_mapping[part[1].color]
            part_pose_msg.part.type = self.type_mapping[part[1].type]

            # storing pose with respect to camera frame
            pose_msg = Pose()
            # !!! INACURATE, part may not always be detected at camera center, but are assumed to
            pose_msg.position.x = float(part[0][0][0])
            pose_msg.position.y = float(part[0][0][1])
            pose_msg.position.z = float(part[0][0][2])
            part_pose_msg.pose = pose_msg
            conveyor_parts_msg.initial_detection = part_pose_msg
            
            # !!! Inacurate, RGB camera cannot detect orientation as of right now. Hardcoded
            pose_msg.orientation.x = 0.0
            pose_msg.orientation.y = 0.0
            if part_pose_msg.part.color == PartMsg.BLUE:
                pose_msg.orientation.z = -0.707107
                pose_msg.orientation.w = 0.707107
            else:
                pose_msg.orientation.z = -0.258819
                pose_msg.orientation.w = 0.965926

            
            # first prediction
            pose_msg.position.x = float(part[0][0][0] + 0.0)
            pose_msg.position.y = float(part[0][0][1] + float(self.CONVEYOR_VEL_Y))
            pose_msg.position.z = float(part[0][0][2] + 0.0)
            part_pose_msg.pose = pose_msg
            conveyor_parts_msg.predictions.append(part_pose_msg)
            msg.conveyor_parts.append(conveyor_parts_msg)

            # second prediction
            pose_msg.position.x = float(part[0][0][0] + 0.0)
            pose_msg.position.y = float(part[0][0][1] + 2*float(self.CONVEYOR_VEL_Y))
            pose_msg.position.z = float(part[0][0][2] + 0.0)
            part_pose_msg.pose = pose_msg
            conveyor_parts_msg.predictions.append(part_pose_msg)
            msg.conveyor_parts.append(conveyor_parts_msg)
            
        self._part_conveyor_pose_publisher.publish(msg)
    ...
    def _conveyor_RGB_camera_cb(self, msg: ImageMsg):
        try:
            self._conveyor_camera_image = self._bridge.imgmsg_to_cv2(msg, "bgr8")
        except CvBridgeError as e:
            print(e)
            ...
            if bin_parts is None:
                ...
                for slot_number, part in bin_parts.items():
                    if part.type is not None:
                        ...
                        parts_bins_poses_wrt_camera = self.partsbinsposes.parts_wrt_camera(parts_bins_pose_wrt_world, camera_pose_wrt_world)
    ...
    def log_parts_on_conveyor(self) -> dict:

        # Obtain details of parts in each bin and update log
        conveyor_parts = self.get_conveyor_parts()

        if not conveyor_parts: # bin parts will be None unitl image processing starts
            self.get_logger().info(f"Waiting for camera imagers ...")
            self.new_part = True
            sleep(1)
        else:
            self.get_logger().info(f"LEN: {len(conveyor_parts)}")
            if len(conveyor_parts) >= 2 and len(conveyor_parts) < 4: # if there is a part detected in view
                if self.new_part:   
                    part = conveyor_parts[0]
                    self.conveyor_parts.append(conveyor_parts[0])
                    

                self.new_part = False

            else: # if the part has gone out of veiw
                self.new_part = True # then prepare to recieve a new part
                return None

        return self.conveyor_parts
    ...
    def get_conveyor_parts(self):
        # Check if the converted image is of Open CV readable Image 
        # seems that for some reason, type is being overriden despite not having a global instance, hence the builtins.type
        if builtins.type(self._conveyor_camera_image) == builtins.type(np.ndarray([])):

            cv_img = self._conveyor_camera_image
            
            # find parts by colour in image
            self.find_parts(cv_img, 'c')
            ...
            for color in self.part_conveyor_poses.keys():
                for type in self.part_conveyor_poses[color].keys():
                    for (csx, csy, _, _) in self.part_conveyor_poses[color][type]:
                        # !!! This has limited accuracy because the assumption is that the object is always detected at the center of the camera and that the part is always offset by 1.129329 along z
                        part.append([[[0, 0, 1.129329], (csx, csy)], PartMsg(color=color, type=type)])
            return part
    ...
    def find_parts(self, img, location):
        '''
        image processing
        ''' 
        # determine if we are looking for bin or conveyor parts
        if location == 'c':
            poses = self.part_conveyor_poses
        else:
            poses = self.part_bin_poses

        # hsv masking
        imgHSV = cv2.cvtColor(img, cv2.COLOR_BGR2HSV)

        for color in poses.keys():
            for type in poses[color].keys():
                ...
                # template matching
                self.matchTemplate(imgMask, color, type, location)

                # display bounding boxes
                if self.display_bounding_boxes:
                    if len(poses[color][type]):
                        # sx, sy -> top left corner
                        # ex, ey -> bottom right corner
                        for (sx, sy, ex, ey) in poses[color][type]:
                            cv2.rectangle(img, (sx, sy), (ex, ey), self.colors[color], 3)
    
    def matchTemplate(self, imgMask, color, type, location):
        # template matching
        if type == "pump":
        ...
        # store results depending on if looking at conveyor or bins
        if location == 'c':
            self.part_conveyor_poses[color][type] = refined_matches
        else:
            ...
            self.part_bin_poses[color][type] = refined_matches
...
class PartsPoses():
    ...
    def parts_wrt_camera(self, part_pose_wrt_world, camera_pose_wrt_world):
